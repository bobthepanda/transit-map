...
export const curveTo = ({control, end, startOffset: relativeControl, endOffset: relativeEndpoint, radius = OFFSET}: NewCurveParameters) => {
    const startCurve: Coordinates = { x: control.x + relativeControl.dx * radius, y: control.y + relativeControl.dy * radius};
    const endCurve: Coordinates = { x: control.x + relativeEndpoint.dx * radius, y: control.y + relativeEndpoint.dy * radius};
